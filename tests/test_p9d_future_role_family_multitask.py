...
from scripts.p9d_future_role_family_multitask import (
    CONTENT_FIELDS,
    ContentCodec,
    FutureRoleFamilyGRU,
    _best_assignment,
...
class FutureContentContractTests(unittest.TestCase):
    ...
    def test_codec_is_train_only_and_target_shapes_are_slotwise(self):
        ...
        self.assertEqual(tuple(CONTENT_FIELDS), ("role", "action_family"))
        self.assertEqual(targets["content_targets"].shape, (1, 5, 5, 2))
        self.assertEqual(int(targets["width_targets"][0, 0]), 2)
        self.assertTrue(np.all(targets["content_targets"][0, 0, 2:] == -1))

    @unittest.skipUnless(FutureRoleFamilyGRU is not None, "torch is unavailable in the local environment")
    def test_forward_and_joint_losses_are_finite(self):
        ...
        model = FutureRoleFamilyGRU([3] * 6, [3] * 10, codec.field_sizes(), dropout=0.0)
