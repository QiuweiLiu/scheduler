#!/usr/bin/env python3
"""NN 增强 ①OOF stacking ②类权重/focal ⑤角色正则(最优特征集)

评估:单级动作族 Top-1(test 562)+ 整体 join/route/layer(test 1,520,next 语义)
对照:XGBoost 单级 0.8950 / 整体 0.8493/0.9520/0.9059
"""
import json
import math
...
def main():
    ...
    def report(name, pb, y, extra=""):
        ...
        print(f"  {name:30s} Top1={t1:.4f} {extra}")

    # ===== ⑤ 角色级正则 =====
    xa = torch.from_numpy(X_all)
    yr_t = torch.from_numpy(y_role)

    def train_role(w, ls, wd, name):
        ...
        for ep in range(80):
            ...
            loss = nn.functional.cross_entropy(net(xa[itr]), yr_t[itr], label_smoothing=ls)
            loss.backward(); opt.step()
            net.eval()
            with torch.no_grad():
                acc = (net(xa[iva]).argmax(1) == yr_t[iva]).float().mean().item()
            if acc > best:
                best, best_state = acc, {k: v.clone() for k, v in net.state_dict().items()}
        net.load_state_dict(best_state); net.eval()
        with torch.no_grad():
            pb = torch.softmax(net(xa[ite]), 1).numpy()
        report(name, pb, yr_t[ite].numpy(), f"(val={best:.3f})")
        return net

    r_plain = train_role(W, 0.0, 0.0, "⑤ 角色MLP(基准,0.8368参照)")
    r_sm = train_role(W, 0.1, 0.0, "⑤ 角色MLP+标签平滑0.1")
    r_wd = train_role(W, 0.0, 1e-4, "⑤ 角色MLP+wd1e-4")
    r_64 = train_role(64, 0.1, 1e-4, "⑤ 角色MLP w64+平滑+wd")
    ...
    # ===== ① OOF stacking(动作族)+ ② 类权重/focal =====
    from sklearn.linear_model import LogisticRegression
    from sklearn.model_selection import KFold
    from collections import Counter

    def build_mlp_fam(loss_mode, w=W):
        net = nn.Sequential(nn.Linear(X_all.shape[1], w), nn.ReLU(), nn.Dropout(0.2),
                            nn.Linear(w, w), nn.ReLU(), nn.Linear(w, 6))
        return net

    def train_fam_mlp(loss_mode):
        net = build_mlp_fam(loss_mode)
        opt = torch.optim.Adam(net.parameters(), lr=1e-3)
        xf = torch.from_numpy(X_all)
        yf = torch.from_numpy(nf_arr)
        wts = None
        if loss_mode == "cw":
            cnt = Counter(nf_arr[nf_tr].tolist())
            wts = torch.tensor([max(cnt) / max(cnt.get(i, 1), 1) for i in range(6)],
                               dtype=torch.float32)
        best, best_state = -1, None
        for ep in range(100):
            net.train(); opt.zero_grad()
            ce = nn.functional.cross_entropy(net(xf[nf_tr]), yf[nf_tr], weight=wts)
            if loss_mode == "focal":
                p = torch.softmax(net(xf[nf_tr]), 1)
                pt = p.gather(1, yf[nf_tr].unsqueeze(1)).squeeze(1)
                ce = ((-(1 - pt) ** 2) * torch.log(pt.clamp(min=1e-9))).mean()
            loss = ce
            loss.backward(); opt.step()
            net.eval()
            with torch.no_grad():
                acc = (net(xf[nf_va]).argmax(1) == yf[nf_va]).float().mean().item()
            if acc > best:
                best, best_state = acc, {k: v.clone() for k, v in net.state_dict().items()}
        net.load_state_dict(best_state); net.eval()
        return net
    ...
    fam_models = {}
    for mode in ("ce", "cw", "focal"):
        net = train_fam_mlp(mode)
        fam_models[mode] = net
        with torch.no_grad():
            pb = torch.softmax(net(xa[nf_te]), 1).numpy()
        report(f"② 动作族MLP {mode}", pb, nf_arr[nf_te])

    # OOF stacking:5 折,MLP(ce)+ XGBoost
    xf = X_all
    yf_np = nf_arr

    def align6(p, classes):
        f = np.zeros((len(p), 6))
        for k, c in enumerate(classes):
            f[:, int(c)] = p[:, k]
        return f

    oof_mlp = np.zeros((len(rows), 6), dtype=np.float32)
    oof_xgb = np.zeros((len(rows), 6), dtype=np.float32)
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    for tr_idx, va_idx in kf.split(nf_tr):
        net = build_mlp_fam("ce")
        opt = torch.optim.Adam(net.parameters(), lr=1e-3)
        xt_ = torch.from_numpy(xf[np.array(nf_tr)[tr_idx]])
        yt_ = torch.from_numpy(yf_np[np.array(nf_tr)[tr_idx]])
        for ep in range(80):
            opt.zero_grad()
            loss = nn.functional.cross_entropy(net(xt_), yt_)
            loss.backward(); opt.step()
        net.eval()
        with torch.no_grad():
            oof_mlp[np.array(nf_tr)[va_idx]] = torch.softmax(
                net(torch.from_numpy(xf[np.array(nf_tr)[va_idx]])), 1).numpy()
        m = xgb.XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=6,
                              random_state=42).fit(xf[np.array(nf_tr)[tr_idx]],
                                                   yf_np[np.array(nf_tr)[tr_idx]])
        oof_xgb[np.array(nf_tr)[va_idx]] = align6(
            m.predict_proba(xf[np.array(nf_tr)[va_idx]]), m.classes_)
    # test 概率
    net = train_fam_mlp("ce")
    with torch.no_grad():
        test_mlp = torch.softmax(net(xa[nf_te]), 1).numpy()
    l2 = xgb.XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=6,
                           random_state=42).fit(X_all[nf_tr], nf_arr[nf_tr])
    test_xgb = align6(l2.predict_proba(X_all[nf_te]), l2.classes_)
    oof_xgb = align6(oof_xgb, np.unique(yf_np[nf_tr]))
    meta = LogisticRegression(max_iter=500).fit(np.hstack([oof_mlp[nf_tr], oof_xgb[nf_tr]]),
                                                yf_np[nf_tr])
    stack_test = meta.predict_proba(np.hstack([test_mlp, test_xgb]))
    report("① OOF stacking(MLP+XGB)", stack_test, nf_arr[nf_te])

    # 整体评估(角色 XGB + 各动作族方案)
    def overall(name, p_fam_te):
        p_fam_map = {i: p_fam_te[k] for k, i in enumerate(nf_te)}
        p_fam_full = np.stack([p_fam_map.get(i, np.zeros(6)) for i in ite])
        m = eval_pipeline(ite, y_role, nf_arr, nm_arr, pr_xgb.argmax(1), p_fam_full)
        results["models"][name + "_整体"] = m
        print(f"  {name:30s} 整体 {m}")

    overall("XGBoost(级联参照)", align6(test_xgb, np.unique(yf_np[nf_tr])))
    overall("① OOF stacking", align6(stack_test, meta.classes_))
    with torch.no_grad():
        overall("② MLP focal", torch.softmax(fam_models["focal"](xa[nf_te]), 1).numpy())

    with open(D + "results/processed/nn_improve_script1_20260807.json", "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print("写入 results/processed/nn_improve_script1_20260807.json")
