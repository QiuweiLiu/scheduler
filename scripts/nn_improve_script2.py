#!/usr/bin/env python3
"""NN 增强 ③FM 显式交互层 + ④FT-Transformer(单级 next 语义 + 整体)

③ MLP+FM:one-hot 块间 embedding 内积(DeepFM 式二阶交互)
④ FT-Transformer:特征 token 化 + 自注意力
对照:XGBoost 单级(next 语义)与整体 join 0.8493
"""
import json
import math
...
def main():
    ...
    def train_net(net, epochs=100):
        opt = torch.optim.Adam(net.parameters(), lr=1e-3)
        best, best_state = -1, None
        for ep in range(epochs):
            net.train(); opt.zero_grad()
            loss = nn.functional.cross_entropy(net(xa[nf_tr]), yf_t)
            loss.backward(); opt.step()
            net.eval()
            with torch.no_grad():
                acc = (net(xa[nf_va]).argmax(1) == yf_v).float().mean().item()
            if acc > best:
                best, best_state = acc, {k: v.clone() for k, v in net.state_dict().items()}
        net.load_state_dict(best_state); net.eval()
        return net, best

    # ③ MLP+FM(one-hot 块间二阶交互)
    class MLPFM(nn.Module):
        def __init__(self):
            super().__init__()
            self.linear = nn.Linear(DIM, W)
            self.fm_v = nn.Parameter(torch.randn(DIM, 8) * 0.1)  # FM 隐向量
            self.head = nn.Sequential(nn.Linear(W + 8, W), nn.ReLU(), nn.Dropout(0.2),
                                      nn.Linear(W, W), nn.ReLU(), nn.Linear(W, 6))
        def forward(self, x):
            h = torch.relu(self.linear(x))
            vx = x.unsqueeze(2) * self.fm_v.unsqueeze(0)  # (B,D,8)
            sq = vx.sum(1) ** 2
            ss = (vx ** 2).sum(1)
            fm = 0.5 * (sq - ss).sum(1, keepdim=True)      # (B,1)
            fmv = self.fm_v.abs().sum(0).unsqueeze(0).expand(x.shape[0], -1)
            return self.head(torch.cat([h, fmv], dim=1))
    net, _ = train_net(MLPFM())
    with torch.no_grad():
        pb = torch.softmax(net(xa[nf_te]), 1).numpy()
    report("③ MLP+FM", pb, nf_arr[nf_te])
    np.save(D + "results/processed/fm_probs_test.npy", pb)

    # ④ FT-Transformer(特征 token 化 + 自注意力)
    class FTTrans(nn.Module):
        def __init__(self):
            super().__init__()
            self.d_model = 64
            self.n_tokens = DIM
            self.tok = nn.Parameter(torch.randn(DIM, self.d_model) * 0.02)
            self.cls = nn.Parameter(torch.randn(1, self.d_model) * 0.02)
            self.attn = nn.MultiheadAttention(self.d_model, 4, batch_first=True)
            self.norm = nn.LayerNorm(self.d_model)
            self.ff = nn.Sequential(nn.Linear(self.d_model, 128), nn.ReLU(),
                                    nn.Linear(128, self.d_model))
            self.head = nn.Sequential(nn.Linear(self.d_model, 64), nn.ReLU(), nn.Linear(64, 6))
        def forward(self, x):
            t = x.unsqueeze(2) * self.tok.unsqueeze(0)   # (B,D,d)
            cls = self.cls.expand(x.shape[0], -1, -1)
            t = torch.cat([cls, t], dim=1)
            a, _ = self.attn(t, t, t)
            t = self.norm(t + a)
            t = self.norm(t + self.ff(t))
            return self.head(t[:, 0])
    net, _ = train_net(FTTrans())
    with torch.no_grad():
        pb = torch.softmax(net(xa[nf_te]), 1).numpy()
    report("④ FT-Transformer", pb, nf_arr[nf_te])
    np.save(D + "results/processed/ftt_probs_test.npy", pb)
    ...
    overall("③ MLP+FM", np.load(D + "results/processed/fm_probs_test.npy"))
    overall("④ FT-Transformer", np.load(D + "results/processed/ftt_probs_test.npy"))

    with open(D + "results/processed/nn_improve_script2_20260807.json", "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print("写入 results/processed/nn_improve_script2_20260807.json")
