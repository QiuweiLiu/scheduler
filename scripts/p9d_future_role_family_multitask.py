#!/usr/bin/env python3
"""Train the P9d future-node role/family multi-task predictor.

This is the first implementation of the approved ``N0/A/B/D`` experiment:

* ``N0`` is the next-step behavior-only reference.
* ``A`` predicts the future layer profile and per-node role/action family.
* ``B`` adds the next-step behavior auxiliary loss.
* ``D`` adds one soft topology-probability conditioning path to the role/family
  decoder.
...
role/family labels.  Each active layer uses fixed slots and one joint
bipartite matching for both future-node fields.  Edges, node IDs, execution
truth, resource truth and scheduler labels are never model inputs.
...
SCHEMA_VERSION = "p9d-future-role-family-multitask-v1"
CONTENT_FIELDS = ("role", "action_family")
VARIANTS = ("N0", "A", "B", "D")
MISSING_NODE = -1
...
def stage0_audit(dataset_root: Path, expected_manifest_sha256: Optional[str] = None) -> Dict[str, Any]:
    ...
    field_values: Dict[str, Set[str]] = {
        field: set() for field in CONTENT_FIELDS + ("raw_action",)
    }
        ...
        for feature, label in zip(features, labels):
            ...
            for layer in layers:
                ...
                for node in nodes:
                    ...
                    field_values["raw_action"].add(raw_action)
...
class ContentCodec:
    """Train-only vocabularies and dense future-node role/family targets."""
...
if torch is not None:
    ...
    class FutureRoleFamilyGRU(nn.Module):
        def __init__(
            self,
    ...
    FutureRoleFamilyGRU = None  # type: ignore[assignment,misc]
...
def _content_metrics(
    ...
    return {
        "oracle_future_node_role_family_macro_f1": float(np.mean(list(oracle_by_field.values()))),
        "predicted_future_node_role_family_macro_f1": float(np.mean(list(predicted_by_field.values()))),
        "structure_error_amplification_gap": float(
            np.mean(list(oracle_by_field.values())) - np.mean(list(predicted_by_field.values()))
        ),
        "oracle_future_node_role_family_macro_f1_by_field": oracle_by_field,
        "predicted_future_node_role_family_macro_f1_by_field": predicted_by_field,
        ...
        metrics.update(
            {
                ...
                "oracle_future_node_role_family_macro_f1": None,
                "predicted_future_node_role_family_macro_f1": None,
                "structure_error_amplification_gap": None,
                "oracle_future_node_role_family_macro_f1_by_field": None,
                "predicted_future_node_role_family_macro_f1_by_field": None,
...
def run(config_path: Path, resume: bool = False) -> Dict[str, Any]:
    ...
    manifest_output = {
        ...
        "source_sha256": {
            "scripts/p9d_future_role_family_multitask.py": sha256_file(Path(__file__)),
            "scripts/p9d_shared_causal_gru.py": sha256_file(PROJECT_ROOT / "scripts/p9d_shared_causal_gru.py"),
            "tests/test_p9d_future_role_family_multitask.py": sha256_file(
