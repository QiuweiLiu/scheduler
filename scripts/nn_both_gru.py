...
def main():
    ...
    yr_t = torch.from_numpy(y_role[itr])
    yr_v = torch.from_numpy(y_role[iva])
    ...
    def train_seq(net, tr_i, va_i, y_t, y_v, epochs=120):
        opt = torch.optim.Adam(net.parameters(), lr=1e-3)
        best, best_state = -1, None
        for ep in range(epochs):
            net.train(); opt.zero_grad()
            loss = nn.functional.cross_entropy(net(sa[tr_i], xa[tr_i]), y_t)
            loss.backward(); opt.step()
            net.eval()
            with torch.no_grad():
                acc = (net(sa[va_i], xa[va_i]).argmax(1) == y_v).float().mean().item()
            if acc > best:
    ...
    results = {"models": {}}
    rnet, rval = train_seq(GRU(4), itr, iva, yr_t, yr_v)
    fnet, fval = train_seq(GRU(6), nf_tr, nf_va, yf_t, yf_v)
    with torch.no_grad():
        pr_role = rnet(sa[ite], xa[ite]).argmax(1).numpy()
        pb_fam = torch.softmax(fnet(sa[nf_te], xa[nf_te]), 1).numpy()
    r_t1 = round(float((pr_role == y_role[ite]).mean()), 4)
    f_t1 = round(float((np.argmax(pb_fam, 1) == nf_arr[nf_te]).mean()), 4)
    print(f"角色GRU Top1={r_t1:.4f} (val={rval:.3f}) | 动作族GRU Top1={f_t1:.4f} (val={fval:.3f})")
    p_map = {i: pb_fam[k] for k, i in enumerate(nf_te)}
    p_full = np.stack([p_map.get(i, np.zeros(6)) for i in ite])
    m = eval_pipeline(ite, y_role, nf_arr, nm_arr, pr_role, p_full)
    print(f"全NN(GRU角色+GRU动作族) 整体 {m}")
    results["models"]["GRU角色_单级"] = {"top1": r_t1, "val": round(rval, 4)}
    results["models"]["GRU动作族_单级"] = {"top1": f_t1, "val": round(fval, 4)}
    results["models"]["全NN_整体"] = m
    with open(D + "results/processed/nn_both_gru_20260807.json", "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print("已写入 results/processed/nn_both_gru_20260807.json")
