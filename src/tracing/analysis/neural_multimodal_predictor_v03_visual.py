...
def visual_key(row:dict[str,Any])->str:
    ctx=row.get("visual_prefix_context") or {}
    video=str(row.get("video_id") or ctx.get("video_id") or "")
    values=[]
    for value in (ctx.get("frame_indices") or []):
        try: values.append(int(value))
        except (TypeError,ValueError): pass
    return video+"|"+",".join(str(x) for x in values)

def load_visual_index(path: str|None)->dict[str,Any]:
    if not path:
        return {"dim":0,"map":{},"metadata":{}}
    payload=torch.load(path,map_location="cpu")
    embeddings=payload.get("embeddings")
    dim=int(payload.get("embedding_dim", embeddings.shape[1] if embeddings is not None else 0))
    mapping={}
    for item in payload.get("metadata") or []:
        idx=int(item["embedding_index"])
        mapping[str(item["key"])]=embeddings[idx].float()
    return {"dim":dim,"map":mapping,"metadata":payload}
...
class V2Dataset(Dataset):
    def __init__(self,rows,artifacts,visual_index=None):
        self.rows=rows; self.a=artifacts
        self.visual_index=visual_index or {"dim":0,"map":{}}
        self.visual_dim=int(self.visual_index.get("dim",0))
    ...
    def __getitem__(self,index):
        ...
        visual=torch.zeros(self.visual_dim,dtype=torch.float32)
        visual_mask=torch.tensor(0.0,dtype=torch.float32)
        if self.visual_dim:
            cached=self.visual_index.get("map",{}).get(visual_key(row))
            if cached is not None:
                visual=cached.float()
                visual_mask=torch.tensor(1.0,dtype=torch.float32)
        targets=[]
        for h in HORIZONS:
        ...
        return {'index':index,'sem':torch.tensor([self.sid(v) for v in sem],dtype=torch.long),
            ...
            'visual':visual,'visual_mask':visual_mask,
...
def collate(batch):
    ...
    return {'index':torch.tensor([x['index'] for x in batch],dtype=torch.long),'sem':sem,'sem_lengths':sem_lengths,
            ...
            'visual':torch.stack([x['visual'] for x in batch]),'visual_mask':torch.stack([x['visual_mask'] for x in batch]),
...
class V2Predictor(nn.Module):
    def __init__(self,artifacts,graph_adj,kind,visual_dim=0):
        super().__init__()
        ...
        self.visual_embedding_branch=nn.Sequential(nn.Linear(max(self.visual_dim,1),128),nn.GELU(),nn.Dropout(0.1))
        self.fusion=nn.Sequential(nn.Linear(128+96+96+64+64+64+128+64+128,256),nn.GELU(),nn.Dropout(0.1))
    ...
    def forward(self,sem,sem_lengths,raw,raw_lengths,text,text_lengths,cats,numeric,visual,visual_mask,teacher):
        ...
        visual_numeric=numeric[:,[self.idx[k] for k in VISUAL_KEYS]]
        ...
        if self.visual_dim:
            visual_state=self.visual_embedding_branch(visual.float())*visual_mask.float().unsqueeze(-1)
        else:
            visual_state=torch.zeros((sem_state.shape[0],128),device=sem_state.device)
        fused=self.fusion(torch.cat([sem_state,raw_state,text_state,self.visual_branch(visual_numeric),self.task_branch(task),self.resource_branch(resource),static,teacher_state,visual_state],dim=-1))
...
def autoregressive_decode(model,row,artifacts,edges,device,visual_index):
    ...
    predicted=[]
    for _ in HORIZONS:
        item=V2Dataset([working],artifacts,visual_index)[0]
        batch=collate([item])
        tensors={k:batch[k].to(device) for k in ('sem','sem_lengths','raw','raw_lengths','text','text_lengths','cats','numeric','visual','visual_mask','teacher')}
        with torch.no_grad():
...
def evaluate(model,rows,artifacts,edges,device,batch_size,visual_index,details_path=None):
    loader=DataLoader(V2Dataset(rows,artifacts,visual_index),batch_size=batch_size,shuffle=False,collate_fn=collate)
    labels=[x for x,_ in sorted(artifacts['label_vocab'].items(),key=lambda z:z[1])]
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for off,index in enumerate(batch['index'].tolist()):
                ...
                paths=beam_decode(start,log_probs,edges,labels,5); beam_predicted=paths[0] if paths else []; predicted_ar=autoregressive_decode(model,row,artifacts,edges,device,visual_index); predicted=beam_predicted
                for h in (1,3,5):
...
def train_one(args,kind):
    rows=read_jsonl(Path(args.input)); visual_index=load_visual_index(args.visual_embeddings or None); artifacts=build_artifacts(rows)
    ...
    seed_everything(args.seed); model=V2Predictor(artifacts,graph_adj,kind,visual_dim=visual_index["dim"]).to(device)
    loader=DataLoader(V2Dataset(train,artifacts,visual_index),batch_size=args.batch_size,shuffle=True,collate_fn=collate)
    ...
    for epoch in range(1,args.epochs+1):
        ...
        val_metrics=evaluate(model,val,artifacts,edges,device,args.batch_size,visual_index)
    ...
    val_metrics=evaluate(model,val,artifacts,edges,device,args.batch_size,visual_index)
    test_metrics=evaluate(model,test,artifacts,edges,device,args.batch_size,visual_index,out_dir/'test_predictions.jsonl')
    report={'schema_version':'multimodal-predictor-v0.3-visual','model':kind,'seed':args.seed,'input':args.input,'best_epoch':best_epoch,
        ...
        'feature_branches':{'semantic_prefix':True,'raw_action_prefix':True,'task_text':True,'structured_visual_evidence':list(VISUAL_KEYS),'internvideo_prefix_embedding':bool(visual_index["dim"]),'internvideo_embedding_dim':visual_index["dim"],'task_numeric':list(TASK_KEYS),'resource_prefix':list(RESOURCE_KEYS),'teacher_prior':True},
        'leakage_contract':{'future_events_excluded':True,'ground_truth_excluded':True,'video_id_used_as_feature':False,
            'remaining_steps_used_as_input':False,'remaining_steps_used_as_supervision_only':True,'compute_events_strictly_before_target':True,'teacher_fit_split':'train_only','visual_embedding_prefix_only':True,'visual_target_and_future_frames_excluded':True},
        'history':history}
...
def main():
    ...
    parser.add_argument('--input',default='/root/autodl-tmp/scheduler/results/processed/neural_prefix_dataset_v0_2/prefixes_visual_context.jsonl')
    parser.add_argument('--output-dir',default='/root/autodl-tmp/scheduler/results/processed/multimodal_predictor_v0_3_visual')
    ...
    parser.add_argument('--visual-embeddings',default='/root/autodl-tmp/scheduler/results/processed/neural_prefix_dataset_v0_2/internvideo_prefix_embeddings.pt')
    args=parser.parse_args()
    kinds=('gru','lstm','gnn') if args.model=='all' else (args.model,)
    reports=[train_one(args,kind) for kind in kinds]
    summary={'schema_version':'multimodal-predictor-summary-v0.3-visual','reports':reports,'config':vars(args)}
